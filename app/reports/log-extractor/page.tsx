...
export default function LogExtractorPage() {
  ...
  const [leaderPrompts, setLeaderPrompts] = useState<Record<number, string>>({})
  const [leaderPromptStatus, setLeaderPromptStatus] = useState<Record<number, LeaderPromptStatus>>({})
  const [leaderPromptsLoading, setLeaderPromptsLoading] = useState(false)
  const [leaderPromptsError, setLeaderPromptsError] = useState<string | null>(null)
  const leaderPromptTimers = useRef<Record<number, ReturnType<typeof setTimeout>>>({})
  ...
  const fetchLeaderPrompts = useCallback(async () => {
    setLeaderPromptsLoading(true)
    setLeaderPromptsError(null)
    try {
      const res = await fetch('/api/log-extractor/leader-prompts', { cache: 'no-store' })
      if (!res.ok) {
        const data = (await res.json()) as { message?: string }
        throw new Error(data.message ?? '加载负责人提示词失败')
      }
      ...
      const statusMap: Record<number, LeaderPromptStatus> = {}

      leaders.forEach((leader) => {
        promptMap[leader.id] = ''
        statusMap[leader.id] = { saving: false, savedAt: null, error: null }
      })

      ;(data.prompts ?? []).forEach((prompt) => {
        promptMap[prompt.supervisorId] = prompt.promptText ?? ''
        statusMap[prompt.supervisorId] = {
          saving: false,
          savedAt: prompt.updatedAt ?? null,
          error: null,
        }
      })

      setLeaderOptions(leaders)
      setLeaderPrompts(promptMap)
      setLeaderPromptStatus(statusMap)
    } catch (error) {
      setLeaderPromptsError(error instanceof Error ? error.message : '加载负责人提示词失败')
      setLeaderOptions([])
      setLeaderPrompts({})
      setLeaderPromptStatus({})
    } finally {
      setLeaderPromptsLoading(false)
    }
  }, [])
  ...
  useEffect(() => {
    if (!authLoaded || !canView) return
    void fetchLeaderPrompts()
  }, [authLoaded, canView, fetchLeaderPrompts])
  ...
  const scheduleLeaderPromptSave = useCallback(
    (leaderId: number, value: string) => {
      if (!canEdit) return
      if (leaderPromptTimers.current[leaderId]) {
        clearTimeout(leaderPromptTimers.current[leaderId])
      }
      leaderPromptTimers.current[leaderId] = setTimeout(async () => {
        setLeaderPromptStatus((prev) => ({
          ...prev,
          [leaderId]: { saving: true, savedAt: prev[leaderId]?.savedAt ?? null, error: null },
        }))
        try {
          const res = await fetch('/api/log-extractor/leader-prompts', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({ supervisorId: leaderId, promptText: value }),
          })
          if (!res.ok) {
            const data = (await res.json()) as { message?: string }
            throw new Error(data.message ?? '保存失败')
          ...
          setLeaderPromptStatus((prev) => ({
            ...prev,
            [leaderId]: { saving: false, savedAt: data.updatedAt ?? null, error: null },
          }))
        } catch (error) {
          setLeaderPromptStatus((prev) => ({
            ...prev,
            [leaderId]: {
              saving: false,
              savedAt: prev[leaderId]?.savedAt ?? null,
  ...
  const handleLeaderPromptChange = (leaderId: number, value: string) => {
    setLeaderPrompts((prev) => ({ ...prev, [leaderId]: value }))
    scheduleLeaderPromptSave(leaderId, value)
  }
  ...
  return (
    <main className={`min-h-screen bg-slate-50 text-slate-900 ${bodyFont.className}`}>
      ...
      <section className="relative mx-auto flex w-full max-w-none flex-col gap-6 overflow-hidden px-4 pb-12 pt-6 sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <section className="flex flex-col rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-xl shadow-slate-200/40 backdrop-blur">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                  {t.panels.logs.title}
            ...
            <div className="mt-5 grid gap-4">
              ...
              <div className="grid h-[520px] gap-3 overflow-y-auto pr-2">
                {logsLoading ? (
                          ...
                          <label className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">
                                {formatDateTime(log.updatedAt, dateLocale)} · {t.logs.photoLabel}{' '}
                                {log.photoCount}
                              </p>
                            </div>
                          </label>
          ...
          <section className="flex flex-col rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-xl shadow-slate-200/40 backdrop-blur">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                  {t.prompt.leaderTitle}
                </p>
                <p className="mt-2 text-sm text-slate-500">{t.prompt.leaderHint}</p>
              </div>
              <div className="text-xs text-slate-500">
                {leaderPromptsLoading ? t.logs.loading : `${leaderOptions.length}`}
              </div>
            </div>

            {leaderPromptsError ? (
              <p className="mt-3 text-xs text-rose-500">{leaderPromptsError}</p>
            ) : null}

            <div className="mt-4 grid h-[520px] gap-3 overflow-y-auto pr-2">
              {leaderPromptsLoading ? (
                <p className="text-sm text-slate-500">{t.logs.loading}</p>
              ) : leaderOptions.length === 0 ? (
                <p className="text-sm text-slate-500">{t.prompt.leaderEmpty}</p>
              ) : (
                leaderOptions.map((leader) => {
                  const status = leaderPromptStatus[leader.id]
                  const statusLabel = status?.saving
                    ? t.prompt.saving
                    : status?.error
                      ? status.error
                      : status?.savedAt
                        ? `${t.prompt.saved} · ${formatDateTime(status.savedAt, dateLocale)}`
                        : t.prompt.saved
                  return (
                    <div key={leader.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                      ...
                      <textarea
                        value={leaderPrompts[leader.id] ?? ''}
                        onChange={(event) => handleLeaderPromptChange(leader.id, event.target.value)}
                        rows={3}
                        placeholder={t.prompt.leaderPlaceholder}
                        disabled={!canEdit}
                        className="mt-3 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm transition focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100 disabled:cursor-not-allowed disabled:bg-slate-100"
                      />
                      {status?.error ? (
                        <p className="mt-2 text-xs text-rose-500">{status.error}</p>
                      ) : null}
                    </div>
                  )
                })
