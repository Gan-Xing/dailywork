...
- 新增“负责人日志”模块：记录施工负责人原始日志，支持按日查看与历史补录，作为后续 AI 汇总输入。
...
- 导出：
  - 保留计划版导出，提供 `Excel` 与 `PDF` 两种格式选择。
  - `status`、收到数量、单价与代付费用用于页面跟踪，不写入导出列；`cancelled` 状态的行不会参与导出。
  - 若关联项目中包含 `邦杜库市政路项目`，则 `Nom/Le temps` 按 `BDK/YYYYMMDD` 输出，而不是直接导出 ISO 日期。
  - `Nom` 列优先根据 `goodsNameKey` / 货物字典输出法语名（例如 `柴油 -> Gasoil`、`水泥 -> Ciment`）；未命中字典时回退原始录入值。
  - Excel 导出保留空白签字位，不渲染签名图片；PDF 导出通过 `UserSignature` 渲染审批人与编制人的图片签名，若没有已激活签名则保持空白，不回退为文字署名。
