...
- New submodule `Tableau de suivi` (`/finance/ledger`) tracks invoice/cheque lifecycle by unique `project + period` case, then fills milestone events over time (no Excel import dependency).
...
- Finance navigation contains two entries: `Entrées` (current accounting rows) and `Tableau de suivi` (ledger lifecycle tracking).
